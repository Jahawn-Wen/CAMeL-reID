...
class PatchEmbed(nn.Module):
    ...
    def forward(self, x):
        ...
        x = self.proj(x).flatten(2).transpose(1, 2)  # B Ph*Pw C （batch_size, h/p * w/p, 96)
...
class SwinTransformer(nn.Module):
    ...
    def forward(self, x):
        x = self.patch_embed(x) #  [B, 3072, 128]

        if self.ape:
            x = x + self.absolute_pos_embed
        x = self.pos_drop(x)  #B L C [30, 3072, 128]

        for i, layer in enumerate(self.layers):
            x = layer(x) #[30, 48, 1024]

        x = self.norm(x)  # B L C [30, 48, 1024]

        x_cls = self.avgpool(x.transpose(1, 2))  # B C 1 [30, 1024, 1]

        x = torch.cat([x_cls.transpose(1, 2), x], dim=1) #B L+1 C [30,49,1024]
