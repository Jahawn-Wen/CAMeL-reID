...
# from models.reid import ClassBlock
...
#     vision_encoder = SGFormer (   
#         img_size=vision_config['image_res'],
#         patch_size=4,  
#         in_chans=3, 
#         embed_dims=vision_config['embed_dim'], 
#         num_heads=vision_config['num_heads'],
#         mlp_ratios=[4, 4, 4, 4],  
#         qkv_bias=True, 
#         depths=vision_config['depths'],
#         sr_ratios=[8, 4, 2, 1],  # 这需要在配置文件中定义
#         num_stages=4,  # 假设 SGFormer 有 4 个阶段
#         drop_rate=0.0,
#         attn_drop_rate=0.,
#         drop_path_rate=0.,
...
#     if load_params:
#         # 这里假设有一个与 SGFormer 兼容的预训练权重文件
#         # download from https://drive.google.com/drive/folders/1ZJDEwxDffqqqzBC2SzhOkpWpGRweDsE4?usp=sharing
#         state_dict = torch.load(vision_config['ckpt'], map_location="cpu")['state_dict_ema']
#         window_size = vision_config['window_size']
...
#     if config.get('reid', False):
#         print("### Do not load pretrained text encoder", flush=True)
#         for key in list(state_dict.keys()):
#             if 'vision_proj' in key or 'text_proj' in key or 'text_encoder' in key:
#                 del state_dict[key]
#         return state_dict
...
#         # Todo
#         sgformer_num_stages = 4  # SGFormer 有 4 个 stages
...
#         elif self.reid:
#             self.classifier = ClassBlock(512, config.get('classes', 751), config.get('itc_dop', 0.5))  # define our classifier.
#             self.criterion = nn.CrossEntropyLoss()
#             if config['lr_2']:
#                 self.init_params.extend(['classifier.' + n for n, _ in self.classifier.named_parameters()])
...
#                 for i in range(1, sgformer_num_stages + 1):
#                     block = getattr(self.vision_encoder, f'block{i}')
#                     for name, param in block.named_parameters():
#                         self.init_params.extend([f'vision_encoder.block{i}.' + name])
...
#         #     itm_loss = self.new_cross_entropy(output, itm_labels)
#         # else:
...
#     if load_params:
#         state_dict = torch.load(vision_config['ckpt'], map_location="cpu")['state_dict_ema']
#         window_size = vision_config.get('window_size', None)  # not used, placeholder
...
#         )
#         if ('init_cross' in config.keys()) and config['init_cross']:
#             init_params.extend(['text_encoder.' + n for n, _ in text_encoder.cls.named_parameters()])  # cross attention cls
#             print("###  init_params.extend --> cross attention  ###")
...
#                 print(f"{k}: {sorted(v)}")
#     else:
#         # 绝大多数训练/微调场景都会开 MLM；若你确实关闭 MLM，则需要改这里：
...
#         self.vision_encoder, vision_width = build_vision_encoder(config, load_params=load_vision_params)
#         self.vision_width = vision_width
#         # SGFormer stages（固定 4）
#         sgformer_num_stages = 4
...
#         self.pool_alpha    = nn.Parameter(torch.tensor([float(config.get('img_pool_alpha_init', -8.0))]))
...
#                         nn.Linear(self.vision_width, self.embed_dim),
#                         nn.GELU(),
...
#         # tokens: [B, N, C]
#         B, N, C = tokens.shape
#         S = int(math.sqrt(N) + 1e-6)
#         if S * S != N or S < 3:
#             return tokens.mean(dim=1)  # 兜底
#         c = S // 2
#         idxs = []
#         for dy in [-1, 0, 1]:
#             for dx in [-1, 0, 1]:
...
#                 q = c + dx
#                 idxs.append(r * S + q)
#         idx = torch.tensor(idxs, device=tokens.device, dtype=torch.long)  # [9]
#         return tokens.index_select(dim=1, index=idx).mean(dim=1)  # [B, C]

#     # ===== 统一的图像 token 池化入口 =====
#     def _pool_image_tokens(self, image_embeds: torch.Tensor) -> torch.Tensor:
#         # image_embeds: [B, N, C]
...
#         elif mode == 'center3x3':
#             return self._center3x3_pool(image_embeds)
#         elif mode == 'mix':
#             # 起步≈CLS（sigmoid(-8)≈0.0003），训练中由数据决定是否加 mean
#             gate = torch.sigmoid(self.pool_alpha)  # 标量
#             return (1.0 - gate) * image_embeds[:, 0, :] + gate * image_embeds.mean(dim=1)
#         else:
#             return image_embeds[:, 0, :]

#     # ===== 仅在取单模态向量时使用池化；跨模态仍走原 token 序列 =====
#     def get_features(self, image_embeds=None, text_embeds=None):
#         if image_embeds is None:
#             text_feat = self.text_proj(text_embeds[:, 0, :])
#             return text_feat
#         elif text_embeds is None:
#             pooled = self._pool_image_tokens(image_embeds)   # [B, C]
#             image_feat = self.vision_proj(pooled)
#             return image_feat
#         else:
#             pooled = self._pool_image_tokens(image_embeds)
#             image_feat = self.vision_proj(pooled)
#             text_feat = self.text_proj(text_embeds[:, 0, :])
#             return image_feat, text_feat
...
class CrossEntropyLabelSmooth(nn.Module):
    """Cross entropy loss with label smoothing regularizer (device-safe)."""
    def __init__(self, epsilon=0.1, use_gpu=True):
        super(CrossEntropyLabelSmooth, self).__init__()
    ...
    def forward(self, inputs, targets):
        log_probs = self.logsoftmax(inputs)
        B, C = log_probs.size()
        idx = targets.to(log_probs.device).long().view(-1, 1)
        one_hot = torch.zeros_like(log_probs)
        one_hot.scatter_(1, idx, 1.0)
        soft = (1.0 - self.epsilon) * one_hot + self.epsilon / C
...
def build_text_encoder(config, vision_width, load_text_params=False, use_mlm_loss=False, config_text=None):
    ...
    if use_mlm_loss:
        ...
        if ('init_cross' in config.keys()) and config['init_cross']:
            init_params.extend(['text_encoder.' + n for n, _ in text_encoder.cls.named_parameters()])
            print("###  init_params.extend --> cross attention  ###")
...
class CAMeL(nn.Module):
    def __init__(self, config=None, load_vision_params=False, load_text_params=False,
        ...
        # === ITM 负采样增强（按 config 控；默认不生效） ===
        self.sim_upper = float(config.get('sim_upper', 1.0))     # <1.0 时启用伪负过滤
        self.neg_topk  = int(config.get('neg_topk', -1))         # >0 时启用 top-k 采样
        self.mlm_stop_grad_image = bool(config.get('mlm_stop_grad_image', False))
        ...
        if ('pa100k_only_img_classifier' in config.keys()) and config['pa100k_only_img_classifier']:
            ...
            self.classifier = ClassBlock(512, config.get('classes', 751), config.get('itc_dop', 0.5))
            self.criterion = nn.CrossEntropyLoss()
            if config['lr_2']:
                self.init_params.extend(['classifier.' + n for n, _ in self.classifier.named_parameters()])
    ...
    def _center3x3_pool(tokens: torch.Tensor) -> torch.Tensor:
        ...
        for dy in [-1, 0, 1]:
            for dx in [-1, 0, 1]:
                ...
                q = c + dx
                idxs.append(r * S + q)
        idx = torch.tensor(idxs, device=tokens.device, dtype=torch.long)
        return tokens.index_select(dim=1, index=idx).mean(dim=1)
    ...
    def _pool_image_tokens(self, image_embeds: torch.Tensor) -> torch.Tensor:
        ...
        if mode == 'cls':
            ...
            gate = torch.sigmoid(self.pool_alpha)
            return (1.0 - gate) * image_embeds[:, 0, :] + gate * image_embeds.mean(dim=1)
        else:
            return image_embeds[:, 0, :]
    ...
    def get_matching_loss(self, image_embeds, image_atts, image_feat, text_embeds, text_atts, text_feat, idx=None):
        """
        Matching Loss with hard negatives —— 加入 sim_upper 过滤 + topk 采样（均由 config 控制）
        """
        bs = image_embeds.size(0)
        ...
        with torch.no_grad():
            # 先以 cosine 作为“真相似度”，再用温度缩放做 softmax 权重
            cos_i2t = image_feat @ text_feat.t()        # [-1, 1]
            cos_t2i = cos_i2t.t()

            sim_i2t = cos_i2t / self.temp
            sim_t2i = cos_t2i / self.temp
            ...
            if self.sim_upper < 1.0:
                m1 = (cos_i2t < self.sim_upper).float()
                m2 = (cos_t2i < self.sim_upper).float()
                weights_i2t = weights_i2t * m1
                weights_t2i = weights_t2i * m2
                weights_i2t = (weights_i2t + 1e-8) / (weights_i2t.sum(1, keepdim=True) + 1e-8)
                weights_t2i = (weights_t2i + 1e-8) / (weights_t2i.sum(1, keepdim=True) + 1e-8)


            def renorm_topk(weights_row):
                if self.neg_topk is not None and self.neg_topk > 0 and self.neg_topk < weights_row.numel():
                    vals, idxs = torch.topk(weights_row, k=self.neg_topk, largest=True)
                    ps = vals / (vals.sum() + 1e-8)
                    return ps, idxs
                else:
                    ps = weights_row / (weights_row.sum() + 1e-8)
                    idxs = torch.arange(weights_row.numel(), device=weights_row.device)
                    return ps, idxs

        # hard negatives 采样
        image_embeds_neg, image_atts_neg = [], []
        for b in range(bs):
            ps, idxs = renorm_topk(weights_t2i[b])
            if torch.all(ps <= 0):
                ps = torch.ones_like(ps) / ps.numel()
            pick = torch.multinomial(ps, 1).item()
            neg = idxs[pick].item()
            image_embeds_neg.append(image_embeds[neg]); image_atts_neg.append(image_atts[neg])
        image_embeds_neg = torch.stack(image_embeds_neg, dim=0)
        image_atts_neg = torch.stack(image_atts_neg, dim=0)

        text_embeds_neg, text_atts_neg = [], []
        for b in range(bs):
            ps, idxs = renorm_topk(weights_i2t[b])
            if torch.all(ps <= 0):
                ps = torch.ones_like(ps) / ps.numel()
            pick = torch.multinomial(ps, 1).item()
            neg = idxs[pick].item()
            text_embeds_neg.append(text_embeds[neg]); text_atts_neg.append(text_atts[neg])
        text_embeds_neg = torch.stack(text_embeds_neg, dim=0)
        text_atts_neg = torch.stack(text_atts_neg, dim=0)
