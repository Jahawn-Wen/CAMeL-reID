...
class SGFormer(nn.Module):
    ...
    def forward_features(self, x):
        ...
        for i in range(self.num_stages):
            ...
            x, H, W = patch_embed(x)
            # x [30, 3136, 64] [30, 784, 128] [30, 196, 256] [30, 49, 512] [B, H/4*W/4, C]
            if i==0:
                x+=self.pos_embed
            # x [30, 3136, 64] [30, 784, 128] [30, 196, 256] [30, 49, 512] [B, H/4*W/4, C]
            for blk in block:
            ...
            # x [30, 64, 56, 56] [30, 128, 28, 28] [30, 256, 14, 14] [30, 49, 512] [B, C, h/4, w/4]
        # cls_token = self.cls_token.expand(B, -1, -1)  # self.cls_token initially [1, 1, C'] [30, 1, 512]
        # #x [30, 49, 512]
        # x = torch.cat([cls_token, x], dim=1)  # Now x is [B, L+1, C'] [30, 50, 512]
    ...
    #         # x.shape (60, 3072, 64) pos_embed(1,9216,64)
    #         if i == 0:
    #             # 确保 x 的形状与 pos_embed 相匹配
    #             _, N, _ = x.shape  #  x 的形状为 [batch_size, num_patches, feature_dim]
    #             if N == self.pos_embed.shape[0]:
    #                 x += self.pos_embed.unsqueeze(0).repeat(B, 1, 1)
    #             else:
    #                 # 如果不匹配，抛出错误或进行适当的调整
    #                 raise RuntimeError("Size mismatch between x and pos_embed")
