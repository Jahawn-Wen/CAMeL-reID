...
from models.model_retrieval import CAMeL_Retrieval
...
from reTools import evaluation_reid

from transformers import BertTokenizer
from re_show import save_numpy_array
...
def main(args, config):
    ...
    model = CAMeL_Retrieval(config=config)
    ...
    if args.task == "itr_icfg":
        ...
        train_dataset, val_dataset, test_dataset = create_dataset('reid', config, args.evaluate)
    else:
    ...
    if args.evaluate:
        ...
        if args.task == "itr_pa100k":
            ...
            score_reid = evaluation_reid(model_without_ddp, test_loader, device)

        else:
            score_test_t2i = evaluation(model_without_ddp, test_loader,
                                                        tokenizer, device, config, args)
        save_numpy_array(score_test_t2i, os.path.join(args.output_dir, "my_array.npy"))
        ...
        if utils.is_main_process():
            ...
            if args.task == "itr_pa100k":
                ...
                print('reid test_result:', flush=True)
                print(score_reid)
        ...
        for epoch in tqdm(range(0, max_epoch)):
            ...
            if utils.is_main_process():
                ...
                if epoch >= 0:  # Starting from epoch 5, save model at every epoch
                    save_obj = {
...
if __name__ == '__main__':
    ...
    yaml = YAML(typ='rt')# 'rt' 表示 round-trip，用于保留大多数格式信息
