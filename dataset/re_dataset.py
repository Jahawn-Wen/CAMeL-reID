...
def random_word_deletion(words, n):
    """
    随机删除非停用词，直到删除了n个词。
    """
    words = words.copy()  # 避免修改原始列表
    non_stop_words_indices = [i for i, word in enumerate(words) if word.lower() not in stop_words]
    random.shuffle(non_stop_words_indices)
    to_delete = non_stop_words_indices[:n]
